...
$\partial_x \vec B = \vec 0$ und $\partial_y \vec B = \vec 0$ gelten muss. Wir setzen das gegebene elektrische Feld dritte Gleichung ein und erhalten als Differentialgleichung:
\[
	c^2
	\begin{pmatrix}
...
\begin{figure}
	\centering
	\begin{tikzpicture}
		\draw[thick, ->] (0, 0) -- (1, -2) node[right] {$\vec E$};
		\draw[thick, ->] (0, 0) -- (2, 1) node[right] {$\vec B$};
		\draw[dotted, ->] (0, 0) -- (-1, 2) node[left] {$\vec E'$};
		\draw[dotted, ->] (0, 0) -- (-2, -1) node[left] {$\vec B'$};
