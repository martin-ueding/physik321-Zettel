...
wir im zweiten Schritt aus, dass $\nabla_i = - \nabla^i$ ist, jedoch
$\nabla_0 = \nabla^0$.
\begin{gather*}
	E_1
	= - \nabla_1 A^0 - \nabla_0 A^1
...
In der Diagrammnotation oder in der Schreibweise $\nabla^{[\mu} A^{\nu]}$, als
...
Die Bianchi-Identität können wir kompakter schreiben als $\nabla^{[\alpha}
...
Wir sollen zeigen, dass sich der obige Ausdruck als totale Divergenz schreiben
lässt, also als
\[
	\nabla_\mu G^\mu
\]
...
Wir beginnen, den Term \eqref{start} zu vereinfachen. Dabei erhalten wir durch
